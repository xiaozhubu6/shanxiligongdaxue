...
import { elderApi, reviewApi } from '@/api'
...
const loadMyElders = async () => {
  try {
    ...
    // 直接获取子女的评价任务（不通过老人API）
    const response = await reviewApi.getChildReviews(user.id)
    ...
    if (currentMonthReviews.length > 0) {
      // 如果有多个评价任务，显示第一个
      currentReview.value = currentMonthReviews[0]
      
      // 获取对应的老人信息
      const elderId = currentReview.value.elderId
      const elderResponse = await elderApi.getElder(elderId)
      selectedElder.value = elderResponse
      
      if (currentReview.value.status === 'COMPLETED') {
        // 如果已完成，加载已提交的评分
    ...
    ElMessage.error('加载评价任务失败')
  }
}

const submitReview = async () => {
  // 检查是否所有问题都已评分（完全照着老人端写）
