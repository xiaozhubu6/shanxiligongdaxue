...
<view>
	...
	<view class="goods-details coup-anim">
		...
		<view class="goods-list" v-if="item.quantity > 0">
			<view class="goods-list-image"><image class="goods-list-image-item" :src="'http://localhost:80'+'/image/dish/'+item.image" mode="aspectFill"></image></view>
			<view class="goods-list-name">
				<view>{{item.name}}</view>
				<view class="list-text">
					<text class="currency-symbol">¥</text>
					<!-- 总价 -->
					<text class="total-price-text">{{item.total_price}}</text>
				</view>
			</view>
...
.goods-list-image .goods-list-image-item{
	display: block;
	width: 130rpx;
	height: 130rpx;
	border-radius: 10rpx;
